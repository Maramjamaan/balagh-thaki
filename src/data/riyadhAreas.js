...
export function findNearestArea(lat, lng) {
  ...
  riyadhAreas.forEach(area => {
    ...
    if (distance < minDistance) { minDistance = distance; nearest = area; }
  });
  return nearest;
}

export function getAreaByName(name) { return riyadhAreas.find(area => area.name === name) || null; }
export function getAllAreas() { return riyadhAreas; }
